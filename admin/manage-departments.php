...
$sql4_row = "SELECT * FROM departments";
...
if (isset($_POST['search-term'])) {
	...
	$sql = "SELECT * FROM `departments` WHERE `name` LIKE  '$search_term' OR `hod` LIKE  '$search_term' OR `faculty` LIKE  '$search_term' LIMIT $start, $rows_per_page";
	$stmt = $conn->prepare($sql);
	$stmt->execute();
	...
	$start = 0;
	$rows_per_page = 5;
	$sql_row = "SELECT * FROM departments";
...
$sql = "SELECT * FROM departments LIMIT $start, $rows_per_page";
	...
	<section id="interface" class="interface">
		...
		<div class="board">
			...
			<div class="add_faculty-btn">
				<a href="./add-department.php" class="btn">Add Data</a>
			</div>
			<table width="100%">
				<thead>
					<tr>
				...
				<tbody>
					<?php foreach ($records as $key => $row) : ?>
						...
						<td><?= $key + 1 ?></td>
								...
								<td><?= $row['faculty'] ?></td>
								<td><?= $row['hod'] ?></td>
								<td><img src="./uploads/<?= $row['hod_sig'] ?>" alt=""></td>
								<td><a href="edit-department.php?id=<?= $row['id'] ?>" class="edit">Edit</a></td>


							<td><a href="manage-departments.php?del_id=<?= $row['id']; ?>" class="delete" onclick="return confirm('Are you sure you want to delete this?')">Delete</a></td>
						</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
