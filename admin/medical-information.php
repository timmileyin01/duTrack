...
if (isset($_POST['search-term'])) {
    ...
    $sql = "SELECT * FROM `medical_information` WHERE `id_number` LIKE  '$search_term' LIMIT $start, $rows_per_page";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    ...
    $start = 0;
    $rows_per_page = 5;
    $sql_row = "SELECT * FROM medical_information";
...
$sql = "SELECT * FROM medical_information LIMIT $start, $rows_per_page";
    ...
    <section id="interface" class="interface">
        ...
        <div class="board">
            ...
            <div class="add_faculty-btn">
                <a href="add-medical-history.php" class="btn">Add Data</a>
            </div>
            <table width="100%">
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($records as $key => $row) : ?>
                        <tr>
                            ...
                            <td><a href="edit-medical-history.php?id=<?= $row['id']; ?>" class="edit">Edit</a></td>
                            <td><a href="medical-information.php?del_id=<?= $row['id']; ?>" class="delete" onclick="return confirm('Are you sure you want to delete this?')">Delete</a></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
