...
if (isset($_POST['search-term'])) {
    ...
    $sql = "SELECT * FROM `documents` WHERE `filename` LIKE  '$search_term' OR `title` LIKE  '$search_term' OR `note` LIKE  '$search_term' OR `id_number` LIKE  '$search_term' LIMIT $start, $rows_per_page";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
$sql = "SELECT * FROM documents LIMIT $start, $rows_per_page";
    ...
    <section id="interface" class="interface">
        ...
        <div class="board">
            ...
            <div class="add_faculty-btn">
                <a href="add-document.php" class="btn">Add Data</a>
            </div>
            <table width="100%">
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php
                    function formatBytes($size1, $precision = 2)
                    {
                        $base = log($size1, 1024);
                    ...
                    <?php foreach ($records as $key => $row) : ?>
                        <tr>
                            ...
                            <td><a href="edit-document.php?id=<?= $row['id']; ?>" class="edit">Edit</a></td>
                            <td><a href="documents.php?del_id=<?= $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this?')" class="delete">Delete</a></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
