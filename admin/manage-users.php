...
if (isset($_POST['search-term'])) {
    ...
    $sql = "SELECT * FROM `users` WHERE `surname` LIKE  '$search_term' OR `firstname` LIKE  '$search_term' OR `othernames` LIKE  '$search_term' OR `id_number` LIKE  '$search_term' LIMIT $start, $rows_per_page";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
...
$sql = "SELECT * FROM users LIMIT $start, $rows_per_page";
    ...
    <section id="interface" class="interface">
        ...
        <div class="board">
            ...
            <div class="add_faculty-btn">
                <a href="./add-user.php" class="btn">Add Data</a>
            </div>
            <table width="100%">
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($records as $key => $row) : ?>
                        <tr>
                            ...
                            <td><?= $row['surname'] ?></td>
                            <td><?= $row['admin'] ?></td>
                            <?php if ($row['admin'] == 'repo_user') : ?>
								<td>
									<form action="add-result.php" method="post">
										<input type="hidden" name="res_id" value="<?= $row['id'] ?>">
										<input type="submit" style="cursor: pointer;" class="btn" value="Add Result" name="add-res_id">
									</form>
								</td>
							<?php else: ?>
                                <td></td>
                                <?php endif; ?>
                            <td><a href="edit-user.php?id=<?= $row['id']; ?>" class="edit">Edit</a></td>
                            <td><a href="manage-users.php?del_id=<?= $row['id']; ?>" class="delete" onclick="return confirm('Are you sure you want to delete this?')">Delete</a></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
