...
if (isset($_REQUEST['import-excel'])) {
    ...
    if ($extension == 'xlsx' || $extension == 'xls' || $extension == 'csv') {
        ...
        foreach ($data as $row) {
            ...
            $existingMedical = selectAll('medical_information', ['id_number' => $id_number]);
            ...
            if ($existingMedical) {
                ...
                $query = mysqli_query($conn, "INSERT INTO medical_information SET id_number='$id_number', history='$history'");
                if ($query) {
                    $success += 1;
                } else {
                    $error1 += 1;
...
<div class="container form-pages">
    ...
    <div class="form">
        ...
        <div class="question" style="display: flex; flex-direction: row; gap: 1rem; align-items: center; justify-content: center;">
            <span class="text-muted">Click to add Bulk Medical Data</span><a href="./add-medical-history.php" class="btn" style="background: blue;">Add Medical Data</a>
        </div>
        <div class="question" style="display: flex; flex-direction: row; gap: 1rem; align-items: center; justify-content: center; margin-top: 1rem;">
            <span class="text-muted">Click to Download Sample File</span><a href="./bulk_sample/medical/sample.xlsx" class="btn" style="background: peach;">Download</a>
        </div>

        <?php include(base_app . "adminHelpers/formErrors.php"); ?>

        <form action="add-bulk-medical.php" method="post" enctype="multipart/form-data">
