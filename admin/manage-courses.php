...
if (isset($_POST['search-term'])) {
	...
	$sql = "SELECT * FROM `courses` WHERE `course_code` LIKE  '$search_term' OR `course_title` LIKE  '$search_term' OR `faculty` LIKE  '$search_term' OR `department` LIKE  '$search_term' LIMIT $start, $rows_per_page";
	$stmt = $conn->prepare($sql);
	$stmt->execute();
...
$sql = "SELECT * FROM courses LIMIT $start, $rows_per_page";
	...
	<section id="interface" class="interface">
		...
		<div class="board">
			...
			<div class="add_faculty-btn">
				<a href="./add-course.php" class="btn">Add Data</a>
			</div>
			<table width="100%">
				<thead>
					<tr>
				...
				<tbody>
					<?php foreach ($records as $key => $row) : ?>
						...
						<td><?= $key + 1 ?></td>
								...
								<td><?= $row['faculty'] ?></td>
								<td><a href="edit-course.php?id=<?= $row['id'] ?>" class="edit">Edit</a></td>

							<td><a href="manage-courses.php?del_id=<?= $row['id']; ?>" class="delete" onclick="return confirm('Are you sure you want to delete this?')">Delete</a></td>
						</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
