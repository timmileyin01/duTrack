...
if (isset($_POST['search-term'])) {
	...
	$sql = "SELECT * FROM `results` WHERE `student_name` LIKE  '$search_term' OR `matric_id` LIKE  '$search_term' LIMIT $start, $rows_per_page";
	$stmt = $conn->prepare($sql);
	$stmt->execute();
...
$sql = "SELECT * FROM results LIMIT $start, $rows_per_page";
	...
	<section id="interface" class="interface">
		...
		<div class="board">
			...
			<div class="add_faculty-btn">
				<a href="./add-result.php" class="btn">Add Data</a>
			</div>
			<table width="100%">
				<thead>
					<tr>
					...
					<?php foreach ($records as $key => $row) : ?>
						<tr>
							...
							<td>
								...
								if ($row['score'] >= 70) {
									echo 'A';
								} elseif ($row['score'] >= 60 && $row['score'] < 70) {
									echo 'B';
								} elseif ($row['score'] >= 50 && $row['score'] < 60) {
									echo 'C';
								} elseif ($row['score'] >= 45 && $row['score'] < 50) {
									echo 'D';
								} else {
									echo 'F';
								}
							...
							<td><?= $row['matric_id'] ?></td>
							<?php $session = selectOne('sessions', ['id' => $row['session_id']]) ?>
							<td><?= $session['session'] ?></td>
							<td><?= $row['semester'] ?></td>
							<td><a href="edit-result.php?id=<?= $row['id'] ?>" class="edit">Edit</a></td>

							<td><a href="manage-results.php?del_id=<?= $row['id']; ?>" class="delete" onclick="return confirm('Are you sure you want to delete this?')">Delete</a></td>
						</tr>
					<?php endforeach; ?>
				</tbody>
			</table>
